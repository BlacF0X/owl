...
// 1. Définir un type pour un objet Email de Clerk
interface ClerkEmail {
  id: string;
  email_address: string;
}

// 2. Définir un type plus précis pour les données de l'utilisateur
interface ClerkUserData {
  id: string;
  first_name: string | null;
  email_addresses: ClerkEmail[];
  primary_email_address_id: string | null;
  created_at: number; // Clerk envoie un timestamp (nombre)
}

// 3. Utiliser notre nouveau type dans ClerkEvent
type ClerkEvent = {
...
export const handleClerkWebhook = async (req: Request, res: Response) => {
  ...
  if (!WEBHOOK_SECRET) {
    return res
      .status(500)
      .json({ message: "Le secret du webhook Clerk n'est pas configuré." });
  }
  ...
  if (!svix_id || !svix_timestamp || !svix_signature) {
    return res
      .status(400)
      .json({ message: 'Erreur : en-têtes de webhook manquants.' });
  }
  ...
  try {
    switch (type) {
      ...
      case 'user.updated': {
        ...
        const primaryEmail = email_addresses.find(
          (emailObj: ClerkEmail) => emailObj.id === primary_email_address_id
        );
    ...
    console.error(
      'Erreur de base de données lors du traitement du webhook :',
      dbError
    );
