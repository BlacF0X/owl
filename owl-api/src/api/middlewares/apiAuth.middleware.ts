...
export const apiAuthMiddleware = (
  ...
  if (!validApiKey) {
    console.error('❌ CRITIQUE : La variable OWL_API_KEY n\'est pas définie sur le serveur.');
  ...
  if (!apiKey || apiKey !== validApiKey) {
    console.warn(`⚠️ Tentative d'accès non autorisée à l'API d'ingestion. IP: ${req.ip}`);
    return res.status(401).json({ message: 'Non autorisé : Clé API invalide ou manquante.' });
  }
