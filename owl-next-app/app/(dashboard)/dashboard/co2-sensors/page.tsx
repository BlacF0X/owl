...
import { 
  Sensor, RoomData, AlertData, 
  EvolutionData, SensorHistoryResponse 
...
const CO2SensorsPage = () => {
  ...
  if (loading) return <div className="flex justify-center h-screen items-center">Chargement...</div>;
  if (error) return <div className="flex justify-center h-screen items-center">Erreur: {error}</div>;
     ...
     <div className="space-y-8 pb-12 w-full px-4 sm:px-6 lg:px-10 pt-6">
      ...
      {bannerAlert && (
        <div className="fixed bottom-6 right-6 max-w-md w-full bg-white border-l-4 border-red-500 rounded-lg shadow-xl z-50 animate-in slide-in-from-bottom-10 p-4 flex items-start gap-4">
            ...
            <button onClick={() => setBannerAlert(null)} className="text-slate-400 hover:text-slate-600">✕</button>
        </div>
      )}
