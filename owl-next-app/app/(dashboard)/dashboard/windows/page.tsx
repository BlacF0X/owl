...
const mockWindowSensors: Sensor[] = [
  { sensor_id: 'win_sensor_001', name: 'Fenêtre du Salon', displayValue: 'Ouvert', state_changed_at: new Date(Date.now() - 1000 * 60 * 45).toISOString(), hub: { hub_id: 'hub_01', name: 'Maison Principale' }, type: { type_key: 'window', name: 'Fenêtre', unit: '-' } },
  { sensor_id: 'win_sensor_002', name: 'Fenêtre de la Chambre', displayValue: 'Fermé', state_changed_at: new Date(Date.now() - 1000 * 60 * 60 * 3).toISOString(), hub: { hub_id: 'hub_01', name: 'Maison Principale' }, type: { type_key: 'window', name: 'Fenêtre', unit: '-' } },
  { sensor_id: 'win_sensor_003', name: 'Fenêtre du Bureau', displayValue: 'Ouvert', state_changed_at: new Date(Date.now() - 1000 * 60 * 5).toISOString(), hub: { hub_id: 'hub_02', name: 'Bureau' }, type: { type_key: 'window', name: 'Fenêtre', unit: '-' } },
  { sensor_id: 'win_sensor_004', name: 'Baie Vitrée Jardin', displayValue: 'Fermé', state_changed_at: new Date(Date.now() - 1000 * 60 * 120).toISOString(), hub: { hub_id: 'hub_01', name: 'Maison Principale' }, type: { type_key: 'window', name: 'Fenêtre', unit: '-' } },
];
...
export default async function WindowSensorsPage() {
  ...
  const sensorsByHub = windowSensors.reduce((acc, sensor) => {
    ...
    return acc;
  }, {} as Record<string, Sensor[]>); // L'accumulateur est un objet où les clés sont des strings et les valeurs des tableaux de Sensor
          ...
          {longestOpenSensor && (
            <div className="flex items-start gap-3 rounded-md border border-orange-200 bg-orange-50 p-3">
              ...
              <div className="text-sm text-orange-800">
                <span className="font-bold">{longestOpenSensor.name}</span> est ouverte depuis le plus longtemps :{' '}
                <span className="font-semibold">{calculateDuration(longestOpenSensor.state_changed_at)}.</span>
              </div>
            </div>
          )}
        </div>
