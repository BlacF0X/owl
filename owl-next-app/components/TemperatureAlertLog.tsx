        ...
        const promises = sensors.map(async (sensor) => {
          try {
            const res = await fetch(
              `${API_URL}/api/sensors/${sensor.sensor_id}/readings?date=${dateStr}`,
              {
        ...
        <div className="overflow-hidden">
          <ul className="divide-y divide-slate-100 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
            {filteredAlerts.map((alert) => (
              <li
                key={alert.id}
                className="py-3 flex justify-between items-center hover:bg-slate-50 transition-colors px-2 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2 h-2 rounded-full ${alert.type === 'high' ? 'bg-red-500' : 'bg-blue-500'}`}
                  ></div>
                  <div>
                ...
                <div
                  className={`text-sm font-bold px-3 py-1 rounded-full ${
                    alert.type === 'high' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'
                  }`}
                >
