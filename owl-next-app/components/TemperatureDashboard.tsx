...
const SensorCard = ({
  ...
  // MODIFICATION ICI : On définit le label dynamique
  const statusLabel = viewMode === 'current' ? 'Température en temps réel' : 'Température maximale';
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center w-full md:w-3/4 animate-in fade-in slide-in-from-bottom-4">
      <div className="w-full md:w-1/3 flex justify-center mb-6 md:mb-0">
        {/* On passe une nouvelle prop 'statusLabel' au composant TemperatureCircle */}
        {/* Si TemperatureCircle n'accepte pas encore cette prop, il faudra le modifier (voir plus bas) */}
        <TemperatureCircle
          sensorName={sensor.name} // Nom propre sans (Max)
          temperature={tempForCircle}
          min={15}
          max={30}
          subtitle={statusLabel} // Nouvelle prop
        />
      </div>
